...
species_matrix <- bracken_data$species_matrix  # Raw Bracken counts (no Human/Salinibacter)

cat("Using raw Bracken species counts (BSI samples only):\n")
...
cat("  Excluded: Homo sapiens, Salinibacter ruber\n\n")

# =============================================================================
# 1. Filter to samples with both Bracken data and antibiotic metadata
# =============================================================================

# Get samples in BOTH species matrix and prepared_data
...
species_use <- species_matrix

# Calculate relative abundance for filtering
species_rel <- species_use / rowSums(species_use)
...
for (abx in top_antibiotics) {
  ...
  tryCatch({
    ...
    # Column names follow pattern: {covariate}:Est, {covariate}:pval.padj
    # For binary variables, may include TRUE suffix: {covariate}TRUE:Est
    ...
    # Debug: print available columns if not found
    if (is.na(target_pval_col) || is.na(target_est_col)) {
      cat("    Available columns:", paste(colnames(aldex_glm), collapse = ", "), "\n")
      cat("    Looking for:", target_col, "\n")
    }
  ...
  tryCatch({
    ...
    # Extract results for target antibiotic
    # Note: fit_data_abundance is a list; $results contains the data frame
...
# Get significant from each method (with safety checks)
sig_aldex <- if (exists("combined_aldex") && nrow(combined_aldex) > 0) {
  get_significant(combined_aldex, "ALDEx2", "pval_BH", "estimate")
} else {
  cat("Warning: No ALDEx2 results available for concordance analysis\n")
  data.frame(antibiotic = character(), species = character(), effect = numeric(), method = character())
}

sig_maaslin <- if (exists("combined_maaslin") && nrow(combined_maaslin) > 0) {
  get_significant(combined_maaslin, "MaAsLin3", "pval_BH", "coef")
} else {
  cat("Warning: No MaAsLin3 results available for concordance analysis\n")
  data.frame(antibiotic = character(), species = character(), effect = numeric(), method = character())
}
...
sig_ancombc <- if (exists("combined_ancombc") && nrow(combined_ancombc) > 0) {
  ...
  cat("Warning: No ANCOM-BC2 results available for concordance analysis\n")
  data.frame(antibiotic = character(), species = character(), effect = numeric(), method = character())
}
